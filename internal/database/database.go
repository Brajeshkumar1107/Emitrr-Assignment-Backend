...
func NewDB(config Config) (*DB, error) {
	...
	if envURL := getenv("DATABASE_URL", ""); envURL != "" {
		connStr = envURL
		log.Println("[DB] Using DATABASE_URL for connection")
	} else {
		connStr = fmt.Sprintf(
			"host=%s port=%d user=%s password=%s dbname=%s sslmode=require",
			config.Host, config.Port, config.User, config.Password, config.DBName,
		)
		log.Println("[DB] Using manual DB config connection string")
	}
	...
	if err := db.PingContext(ctx); err != nil {
		log.Printf("[DB-ERROR] Database ping failed: %v", err)
		return nil, fmt.Errorf("error connecting to the database: %v", err)
	}

	log.Printf("[DB] ✅ Connected to database successfully")
	return &DB{db}, nil
}
...
func (db *DB) UpdateGameResult(ctx context.Context, gameID, winnerID int, gameState map[string]interface{}) error {
	...
	_, err = tx.ExecContext(ctx, `
		...
		SET games_played = p.games_played + 1,
			games_won = CASE WHEN p.id = $1 THEN p.games_won + 1 ELSE p.games_won END
		FROM games g
		WHERE g.id = $2
		  AND (
			(g.is_bot_game = TRUE AND p.id = g.player1_id)
			OR (g.is_bot_game = FALSE AND p.id IN (g.player1_id, g.player2_id))
		  )`,
		winnerID, gameID,
	)
	if err != nil {
